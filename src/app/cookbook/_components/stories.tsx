            ...
            >
              ...
              <div className={styles.storyCopy}>
                ...
                {story.acknowledgements && (
                  <section className={styles.summaries} aria-labelledby={`${story.id}-thanks-heading`}>
                    <h4 id={`${story.id}-thanks-heading`} className={styles.textHeading}>With special thanks</h4>
                    {story.acknowledgements.map(note => (
                      <section key={note.id} className={styles.summary} aria-labelledby={`${story.id}-${note.id}-heading`}>
                        <h5 id={`${story.id}-${note.id}-heading`} className={styles.textHeading}>{note.title}</h5>
                        <ExpandableText id={`${story.id}-${note.id}`} paragraphs={note.paragraphs} />
                      </section>
                    ))}
                  </section>
                )}
